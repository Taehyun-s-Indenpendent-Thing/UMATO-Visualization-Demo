...
function randomCircleData(numPoints) {
  ...
  return points;
    ...
    function linkedClusters(nClusters, perCluster, perLink, dim) {//1,2
      const points = [];
      const centroids = [];
...
`;
    ...
    function decideMyPoints(options, globalParams, demoName) {
      ...
      const paramFolder = globalParams.join('_');
    ...
    function instance$4($$self, $$props, $$invalidate) {
      ...
      var delay = 0;
      var delay = 0;
      for (var i = 40; i <= 200; i=i+40) {
          for(var j = 3; j <= 12; j=j+3){
            for(var k = 5; k <= 25; k=k+10){ 
              delay += 200;
              setTimeout(function(i_local, j_local,k_local) {
                  return function() {
                      var pointsObject = clusteredLineImages(i_local, j_local,0,k_local);
                      var pointsText = JSON.stringify(pointsObject);
                      var filename = i_local + "_" + j_local + "_0_"+k_local+'.txt';
                      var blob = new Blob([pointsText], { type: 'text/plain' });
                      var link = document.createElement('a');
                      link.download = filename;
                      link.href = URL.createObjectURL(blob);
                      link.click();
      ...
      */
      ...
      function optimalParams(demoName) {
        ...
        const optimalParamsMap = {
          ...
          'Equally Spaced': { nNeighbors: 35, minDist: 1, hub_num: 40 },
